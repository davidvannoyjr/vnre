...
- **Offer Summary / Net Sheet** (offer-to-spreadsheet skill) — 4-tab xlsx built from scratch by `build_workbook.py`: Offer Comparison/Summary, Net Proceeds, Team Commission Breakdown, Commission Letter (commission tabs build only when the JSON has a `commission` object). 1 offer → "Offer Summary" (no #1/#2, no best-term highlight); 2+ → "Offer Comparison" (per-offer columns, best-term highlight). Net Proceeds links auto-retarget to whichever tab name applies — never hardcode the source tab name. Palette: neutral base (steel #2A2A2A header, #EDEDED bands, #F4F4F4 strip), red #C8102E as accent ONLY (Net-to-Seller hero, best-term highlights, read-full-offer flag, before-payoff qualifier, Amount Due Co-op / Net to Agent). Full brand hexes: RED C8102E · CHARCOAL 1C1C1C · GRAY 6E6E6E · WHITE FFFFFF; row shading F4F4F4, label band E8E8E8, TBD-flag YELLOW FFF3B0, best-term tint HILITE FBE3E6. Fonts: Roboto Condensed Bold headers, Roboto body; client-facing numbers in Roboto (intentional deviation from the Mono-for-stats brand rule, for a cleaner client feel), internal tabs in Roboto Mono. DVN-changed inputs: blue text + yellow fill; unfound values "TBD" + yellow fill (never invented). Logo sits on a white header band (the dark+red asset is invisible on dark) and falls back to the "VAN NOY REAL ESTATE" wordmark if the asset is missing. Always recalc and confirm zero formula errors before delivering.
...
| `vnre-database-coi` | "run the Database & COI brief", "equity and referral check" | `04 Tools/database-coi-skill/SKILL.md` (aiDrVN Stage 08; source + sample in repo `tools/vnre/database-coi-skill/`) |
| `vnre-clv-sync` | "sync CLV", "client lifetime value", "partner value" | `04 Tools/clv-sync-skill/SKILL.md` (CLV from closings → FUB; QBO partner-value brief; source in repo `tools/vnre/clv-sync-skill/`) |
| `vnre-ceo-dashboard` | "CEO dashboard", "run my numbers", "where am I vs plan" | `04 Tools/ceo-dashboard-skill/SKILL.md` (aiDrVN Stage 07; QuickBooks actuals vs 2026 plan + margin guard; source in repo `tools/vnre/ceo-dashboard-skill/`) |
| `vnre-book-appointment` | "book the LA for {name}", "add this listing appointment" | `04 Tools/stage01-prospecting/SKILL.md` (aiDrVN Stage 01 booking → LA: event → PLP pipeline; source in repo `tools/vnre/stage01-prospecting/`) |
| `vnre-active-hunter` | "build my call list", "who do I prospect today" | `04 Tools/active-hunter-skill/SKILL.md` (aiDrVN Stage 01 daily targeting + DVN-voice script library; source in repo `tools/vnre/active-hunter-skill/`) |
...
| `vnre-fob-mpc-top-leases` | "fob MPC check", "MPC check", "top leases this week", "lease listings", "what leases came in" | `tools/vnre/fob-mpc-top-leases-skill/SKILL.md` (FUB deals → top 3 lease listings this week, ranked by price; source in repo `tools/vnre/fob-mpc-top-leases-skill/`) |
...
### Homelight Leads Campaign
Homelight lead follow-up reference material — campaign assets and follow-up cadence for Homelight-sourced leads. Reference project; stays in `01 Projects/`.
